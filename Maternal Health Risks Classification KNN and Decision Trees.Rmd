...
The miscalculation is 38% meaning the **accuracy of entire KNN model is 62%**. This is not optimal.
...
Decision Trees are another way for classification. It is a tree-like flowchart that is used to decide how to classify an observation. With every decision there will be a "yes/no" and branch out to the next node and will continue to do so until all the metrics are used. Thus, similar to KNN we will use the metrics to predict the classification a person is one of the 3 RiskLevel types.
```{r}
